...
final class GroqButler
{
    ...
    {
        if (! config('resumo.groq.enabled')) {
            throw new \RuntimeException('Groq is disabled in .env.');
        }

        $apiKey = trim((string)config('resumo.groq.api_key', ''));
        if ($apiKey === '') {
            throw new \RuntimeException('Groq API key is missing in .env.');
        }

        $baseUrl = rtrim((string)config('resumo.groq.base_url'), '/');
        $model = trim((string)config('resumo.groq.model'));
        $timeout = max(15, (int)config('resumo.groq.timeout'));
        $maxTokens = min(900, max(220, (int)config('resumo.groq.butler_max_tokens')));
