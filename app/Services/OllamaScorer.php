...
final class OllamaScorer
{
    ...
    {
        if (! config('resumo.ollama.enabled')) {
            self::$lastStatus = 'Ollama is disabled in .env.';
            return null;
        }

        $baseUrl = rtrim((string)config('resumo.ollama.url'), '/');
        $url = $baseUrl . '/api/generate';
        $model = config('resumo.ollama.model');
        $fallbackModel = trim((string)config('resumo.ollama.fallback_model', ''));
        $timeout = max(30, (int)config('resumo.ollama.timeout'));
        $context = max(1024, (int)config('resumo.ollama.context'));
        $numPredict = max(420, (int)config('resumo.ollama.num_predict'));
